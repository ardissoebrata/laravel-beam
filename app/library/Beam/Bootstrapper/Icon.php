...
class Icon extends Tag
{
   ...
   /**
    ...
    public static function __callStatic($method, $parameters)
    {
        ...
        // Concatenate icons
		$icon_prefix = Helpers::getContainer('config')->get('bootstrapper::icon_prefix');
        $classes = $icon_prefix . implode('-', $classes);
        if ($white) $classes .= ' ' .Helpers::getContainer('config')->get('bootstrapper::icon_prefix').'white';
        $classes = substr($icon_prefix, 0, strlen($icon_prefix) - 1) . ' '.$classes;
